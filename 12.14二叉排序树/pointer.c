...
/*二叉排序树中插入结点*/
int insert(PBinSearchTree ptree, KeyType key,DicElement *ele) {
...
int creatSearchTree(PBinSearchTree ptree, SeqDictionary *dic) {
	...
	*ptree = NULL; 			/* 将二叉排序树置空 */
	for (i = 0; i<dic->n; i++)
		if (!insert(ptree, dic->element[i].key,&dic->element[i]))//注意插入的时候是DicElement的指针类型，所以加&
			return 0;			/* 将新结点插入树中 */
	return 1;
}
...
void main() {
	...
	while(1){
		...
		if(way==1){
			...
			if(res!=0){ 
				printf("find is ok!");
				//打印的时候可以直接通过树中节点的指针获取对应字典的key和value
				printf("key: %d  classinfo:%s   name:%s\n",pos->element->key,pos->element->value->info,pos->element->value->name);
			}
